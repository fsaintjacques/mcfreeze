...
func startTestServer(t *testing.T) (*Server, *AssignmentBroker) {
	t.Helper()
	broker := NewAssignmentBroker()
	srv, err := NewServer(broker, "127.0.0.1:0")
	if err != nil {
		t.Fatal(err)
	}
	go srv.Serve()
...
func TestServer_PostState(t *testing.T) {
	...
	got, ok := broker.GetNodeState("node-1")
...
func TestServer_LongPoll(t *testing.T) {
	...
	broker.SetAssignments("node-1", nil)
	...
	broker.SetAssignments("node-1", []api.NodeAssignment{{
